...
public:
	...
	void Load(physics::ModelPtr parent, sdf::ElementPtr sdf)
	{
		...
		rkP = model -> GetWorldPose().rot;
		rkQ = model -> GetWorldPose().rot;
		rkPN = model -> GetWorldPose().rot;
		rkQN = model -> GetWorldPose().rot;
...
private:
	...
	{
		math::Quaternion q;
		math::Quaternion mq =  model -> GetWorldPose().rot;
		q.SetFromEuler(0.0,0.0,angle - mq.GetYaw());
		rkQ = q;
		rkQ.Normalize();
	}
	...
	void OnUpdate()
	{
		...
		math::Pose poseKinectR = model -> GetLink(linkPrefix + "ownKinect::" + "kinect_base") -> GetWorldPose();
		poseKinectR.rot = model->GetWorldPose().rot * rkQ;
	...
	gazebo::math::Quaternion rkP;
	gazebo::math::Quaternion rkQ;
	gazebo::math::Quaternion rkPN;
	gazebo::math::Quaternion rkQN;
	double ft = 0.1;
