...
#include "head_pose_estimation_profile.hpp"
...
HeadPoseEstimationProfile::HeadPoseEstimationProfile(const string& face_detection_model, float focalLength) :
...
std::vector<std::vector<Point>> HeadPoseEstimationProfile::update(cv::InputArray _image)
{
...
head_pose HeadPoseEstimationProfile::pose(size_t face_idx) const
{
    ...
    head_points.push_back(P3D_SELLION_PROFILE);
    //head_points.push_back(P3D_RIGHT_EYE);
    head_points.push_back(P3D_LEFT_EYE_PROFILE);
    head_points.push_back(P3D_LEFT_EYEBROW_PROFILE);
    //head_points.push_back(P3D_RIGHT_EAR);
    //head_points.push_back(P3D_LEFT_EAR);
    head_points.push_back(P3D_MENTON_PROFILE);
    head_points.push_back(P3D_NOSE_PROFILE);
    head_points.push_back(P3D_STOMMION_PROFILE);

    std::vector<Point2f> detected_points;

    detected_points.push_back(coordsOf(face_idx, SELLION_PROFILE));
    detected_points.push_back(coordsOf(face_idx, LEFT_EYE_PROFILE));
    detected_points.push_back(coordsOf(face_idx, LEFT_EYEBROW_PROFILE));
    ...
    detected_points.push_back(coordsOf(face_idx, MENTON_PROFILE));
    detected_points.push_back(coordsOf(face_idx, NOSE_PROFILE));

    auto stomion = (coordsOf(face_idx, SELLION_PROFILE) + coordsOf(face_idx, MENTON_PROFILE)) * 0.5;
...
std::vector<head_pose> HeadPoseEstimationProfile::poses() const 
{
...
Mat HeadPoseEstimationProfile::drawDetections(const cv::Mat& original_image, const std::vector<std::vector<Point>>& detected_features, const std::vector<head_pose>& detected_poses) {
...
void HeadPoseEstimationProfile::drawFeatures(const std::vector<std::vector<Point>>& detected_features, Mat& result) const {
...
void HeadPoseEstimationProfile::drawPose(const head_pose& detected_pose, size_t face_idx, cv::Mat& result) const {
   ...
   head_points.push_back(P3D_SELLION_PROFILE);
    ...
    putText(result, "(" + to_string(int(detected_pose(0,3) * 100)) + "cm, " + to_string(int(detected_pose(1,3) * 100)) + "cm, " + to_string(int(detected_pose(2,3) * 100)) + "cm)", coordsOf(face_idx, SELLION_PROFILE), FONT_HERSHEY_SIMPLEX, 0.5, text_color,2);
}

Point2f HeadPoseEstimationProfile::coordsOf(size_t face_idx, FACIAL_FEATURE_PROFILE feature) const
{
...
bool HeadPoseEstimationProfile::intersection(Point2f o1, Point2f p1, Point2f o2, Point2f p2,
                                      Point2f &r) const
{
    Point2f x = o2 - o1;
    Point2f d1 = p1 - o1;
